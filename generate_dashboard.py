...
def get_fred(series_id):
    """FRED APIから最新の (値, 観測日) を取得。失敗時 (None, None)。
    v0.3 E-1: 死系列(次回観測が来ない)を観測日で検知できるよう日付も返す。"""
...
def buffett_us_selfcalc():
    """米国バフェット指数を公的統計から自前算出（旧 FRED DDDM01USA156NWDB は
    2020終了の死系列のため使用しない）。
      NCBEILQ027S（米企業株式時価総額 $百万・四半期）
      ÷ (GDP（米名目GDP $十億・四半期） × 1000) × 100
    両系列とも最新観測四半期を使用（恣意的な期合わせをしない）。失敗時 None。"""
    mc, _ = get_fred('NCBEILQ027S')
    gd, _ = get_fred('GDP')
    if mc and gd and gd > 0:
        return round(mc / (gd * 1000) * 100, 1)
    return None
...
def load_valuation():
        ...
        if len(rows) >= 2:
            ...
            if rows[1][0].startswith(today):
                ...
                return {
                    ...
                    # バフェット: キャッシュ(旧誤値の可能性)を信用せず常に自前算出/非表示
                    'buffett_jp': None,
    ...
    prev = {
        ...
        # 旧 195/133 は2020終了の死系列値。誤値再表示を防ぐため None（米は自前算出/日は非表示）
        'buffett_jp': None, 'buffett_us': None, 'usdjpy': 158,
    }
    ...
    rate_us, _ = get_fred('DGS10')
    rate_jp, _ = get_fred('IRLTLT01JPM156N')
    # 米国＝公的統計から自前算出（旧 DDDM01USA156NWDB は2020終了の死系列）
    buffett_us = buffett_us_selfcalc()
    # 日本＝信頼できる生系列なし → 誤った2020値を出さず非表示（販売後 堅牢ソース整備）
    buffett_jp = None
    ...
    # ── 総合判定 ────────────────────────────────────────
    # v0.3 B-1: 日本バフェットは非表示のため判定から除外。4指標で比較し、
    # 原仕様の閾値(5指標中4/3/2=80/60/40%)を比率で維持（不正反転防止）。
    def _cmp(a, b, gt):
        if not a or not b: return None
        return (a > b) if gt else (a < b)
    _adv = [
        _cmp(pbr_jp,   pbr_us,   False),   # 日本PBR < 米国PBR
        _cmp(per_jp,   per_us,   False),   # 日本PER < 米国PER
        _cmp(div_jp,   div_us,   True),    # 日本配当 > 米国配当
        _cmp(yield_jp, yield_us, True),    # 日本益回り > 米国益回り
    ]
    _valid = [x for x in _adv if x is not None]
    _n     = len(_valid) or 1
    ...
    result = {
        ...
        'rate_diff': rate_diff,
        'buffett_jp':buffett_jp,
        'buffett_us':buffett_us,
...
def fetch_market():
    ...
    gold = yp('GC=F')
    try:
        _t10v, _ = get_fred('DGS10'); t10 = _t10v or 4.3
        _t2v,  _ = get_fred('DGS2');  t2  = _t2v  or 4.5
...
# SHORT_SCORE（短期1年）= LayerA(リスク) + LayerB(金融) を0-100に正規化（満点70）
# MID_SCORE（中期3年）= LayerC(経済) を0-100に正規化（満点20）。Layer D(バリュ) は
#   STEP5 C で総合から除外済のため中期からも除外（2026-05-19 副作用是正・案A）
try:
    ...
    # 中期 = LayerC のみ / 20 * 100。Layer D は STEP5 C で総合除外済のため中期からも
    #   除外し、分母を実満点20に是正（短期の /70 正規化と同一設計・2026-05-19 案A）
    MID_SCORE   = int(round(_lc / 20 * 100))
except:
...
# 保有+監視のみ厳格チェック（予測記録連動で銘柄固有値が期待される）。
# スクリーニング(Top75)は予測記録未登録のため proxy計算となり、
# 市場環境によっては値が揃いやすいため、ガードは警告のみ（停止しない）。
_guard_cell_diversity(rows_h + rows_w, sheet_name='保有+監視')

def _warn_cell_diversity(rows, col_idx_short=3, col_idx_mid=4, sheet_name=''):
    """停止せずWARNログのみ"""
    import re as _re
    def _extract_td_texts(tr_str):
        cells = _re.findall(r'<td[^>]*>(.*?)</td>', tr_str, flags=_re.DOTALL)
        return [_re.sub(r'<[^>]+>', '', c).strip() for c in cells]
    if len(rows) < 10:
        return
    from collections import Counter
    shorts, mids = [], []
    ...
    for label, vals in (('短期', shorts), ('中期', mids)):
        ...
        same_ratio = top_n / len(vals)
        if same_ratio > 0.80:
            print(f"  WARN {sheet_name} の{label}列: '{top_val}' が "
                  f"{top_n}/{len(vals)} ({same_ratio:.0%})。予測記録未登録のため proxy 計算")

_warn_cell_diversity(rows_s, sheet_name='スクリーニング')
...
# ── 企業秘密保護（2026-06-09・CEO指示）: 顧客向けヘルプ詳細と詳細パネルの式を非表示化 ──
# スコア計算式・ウェイト・閾値・導出方法は企業秘密。fetch した src に対し毎回冪等に適用する。
# ① "?" ポップアップ6種 → 「あいまいな高レベル説明＋将来保証しない」だけに削る
# ② 詳細パネル d-matrix-note の式『総合○点（ランク）= Real ROIC…×40%…』を削除し総合評価コメントのみ残す
# 対象外（CEO「それ以外は指示していない」）: レーダー図 / 3変数バー / マトリックス図 / 非表示の v4.3: ソース
_HELP_VAGUE = {
    'v42': '\n<p>企業の本質的価値を独自に相対評価し、S/A/B/C/D のランクに分類した参考評価です。</p>\n<div class="note">将来のリターンや勝率を保証するものではありません。市場環境とともに毎月見直されます。</div>\n',
    'short_score': '\n<p>市場全体の短期的な環境（市場の体温）を数値化した指標です。各銘柄カードの短期は、この体温を起点に、その銘柄の割安度で補正した銘柄別の参考値です。</p>\n<div class="note">将来を保証する予測ではありません。市場環境とともに毎月見直されます。</div>\n',
    'medium_score': '\n<p>市場全体の中期的な環境（おおむね1〜3年）を数値化した指標です。各銘柄カードの中期は、この環境を起点に、その銘柄の業績トレンドで補正した銘柄別の参考値です。</p>\n<div class="note">将来を保証する予測ではありません。市場環境とともに毎月見直されます。</div>\n',
    'short': '\n<p>市場の短期的な環境を起点に、各銘柄の割安度・業種特性で補正した銘柄別の参考方向です（検証中・参考）。ラベル（強気〜弱気）は相対的な強弱の目安にすぎません。</p>\n<div class="note">毎月見直されます。将来を保証する予測ではありません。</div>\n',
    'medium': '\n<p>市場の中期的な環境を起点に、各銘柄の業績トレンドで補正した銘柄別の参考方向です（検証中・参考）。ラベルは相対的な強弱の目安にすぎません。</p>\n<div class="note">毎月見直されます。将来を保証する予測ではありません。</div>\n',
    'long': '\n<p>企業の本質的価値を中心とした相対評価（ランク）にもとづく長期の目安です。</p>\n<div class="note">個別銘柄の将来のリターンを示すものではなく、市場環境とともに毎月見直されます。</div>\n',
}
for _hk, _hv in _HELP_VAGUE.items():
    src, _hn = re.subn(r"(\b" + _hk + r":\{\s*title:'[^']*',\s*html:`)(.*?)(`)",
                       lambda m, _v=_hv: m.group(1) + _v + m.group(3),
                       src, count=1, flags=re.DOTALL)
    if _hn == 0:
        print(f"  ::warning::HELP['{_hk}'] 未検出（あいまい化スキップ・パターン要確認）")
_dm_formula = "総合${v42}点（${rk}ランク）= Real ROIC ${s1}pt×40% + トレンド ${s2}pt×35% + 価格 ${s3}pt×25%。"
if _dm_formula in src:
    src = src.replace(_dm_formula, "", 1)
else:
    print("  ::warning::詳細パネルの式 未検出（既に削除済 or パターン要確認）")
print("OK: ヘルプ詳細/詳細パネルの式を非表示化（企業秘密保護）")
...
MACRO_CARDS_HTML = ('    <div class="macro-cards-grid" style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:2px;">\n'
    f'      {card1}\n      {card2}\n      {card3}\n      {card4}\n    </div>')
...
buf_jp  = VAL['buffett_jp']           # None=非表示（v0.3 B-1：日本は堅牢ソース無し）
buf_us  = VAL['buffett_us']           # None=取得不可（FRED障害時）
_buf_us_ok = isinstance(buf_us, (int, float))
buf_us_disp = f"{buf_us:.0f}%" if _buf_us_ok else "—"
_buf_us_js  = f"{buf_us:.0f}" if _buf_us_ok else "null"
...
g_buf_us  = make_gauge(80,260,buf_us,120,170) if _buf_us_ok else ''
...
buf_us_cls  = ('g' if buf_us<=120 else 'y' if buf_us<=170 else 'r') if _buf_us_ok else 'y'
...
buf_us_lbl  = ('割安圏' if buf_us_cls=='g' else '割高圏注意' if buf_us_cls=='y' else '割高警戒') if _buf_us_ok else 'データ取得中'
...
VAL_HTML = f"""        <div class="sl">バリュエーション — 日本 vs 米国（過去10年との比較）<span style="font-size:var(--fs-micro);color:#34d399;font-weight:600;margin-left:8px;">✓ {updated_display}</span></div>
        ...
        <div class="val-macro-grid" style="display:grid;grid-template-columns:repeat(5,1fr);gap:0;">
          <div style="{vi_style}" onclick="showVI('cape')">
            <div style="{vn_style}">シラーPER（米CAPE） ⓘ</div>
            ...
            <div style="font-size:var(--fs-micro);color:#64748b;margin:2px 0 1px;">🇯🇵は日本PER×1.3の簡易参考値（10年平均利益のCAPEではありません）</div>
            <div style="margin-top:3px;display:flex;justify-content:space-between;">{badge('日本 '+cape_jp_lbl,cape_jp_cls)}{badge('米国 '+cape_us_lbl,cape_us_cls)}</div>
            <div style="font-size:var(--fs-micro);color:#475569;margin-top:2px;">出典: 米=multpl.com（CAPE実測）/ 日=日経PER×1.3 簡易</div>
          </div>
          ...
          <div style="{vi_style}" onclick="showVI('buffett')">
            ...
            <div style="{row_style}"><span style="{flag_style}">🇺🇸</span><span style="{vc[buf_us_cls]}">{buf_us_disp}</span><div style="flex:1">{g_buf_us}</div></div>
            <div style="font-size:var(--fs-micro);color:#64748b;margin:3px 0 1px;">🇯🇵 日本：信頼できる最新データの整備が整うまで一時非表示（準備でき次第ご提供）</div>
            <div style="margin-top:3px;display:flex;justify-content:flex-start;">{badge('米国 '+buf_us_lbl,buf_us_cls)}</div>
            <div style="font-size:var(--fs-micro);color:#475569;margin-top:3px;">出典: 米FRB/BEA 公的統計（株式時価総額÷名目GDP・四半期）</div>
          </div>
...
<script>
var VI_DATA={{
  cape:{{ttl:'シラーPER（CAPE）とは',body:'過去10年の平均利益で計算した株価収益率です。\\n景気の良い年・悪い年を平均するため、より正確に割高・割安を判断できます。\\nノーベル賞を受賞したシラー教授が考案。\\n\\n米国はmultpl.comのCAPE実測値です。\\n日本は10年平均利益データが未整備のため、日本PER×1.3の簡易参考値であり正式なCAPEではありません。\\n\\n目安：15倍以下→割安 / 22〜28倍→適正 / 35倍超→割高',jp:{{min:15,max:35,now:{cape_jp:.1f},g:22,w:28,flag:'🇯🇵 日本(簡易)'}},us:{{min:22,max:45,now:{cape_us:.1f},g:27,w:35,flag:'🇺🇸 米国(CAPE)'}}}},
...
# ── モバイル対応（案C・2026-05-22 CEO 指示・手順書 §5）─────────────
# viewport meta を保証（冪等）。無いとスマホは PC 幅で縮小表示される。
if '<meta name="viewport"' not in src:
    src = src.replace(
        '<meta charset="utf-8">',
        '<meta charset="utf-8">\n<meta name="viewport" content="width=device-width, initial-scale=1">',
        1)
    print("OK: viewport meta 注入（モバイル対応）")
else:
    print("OK: viewport meta は既に存在")

# レスポンシブ CSS（冪等・id="responsive-mobile" で区間置換）。
# 末尾 <style> として注入し、main <style> の後に来るため @media が確実に効く。
RESPONSIVE_CSS = (
    '<style id="responsive-mobile">\n'
    '/* モバイル対応 案C 2026-05-22 手順書§5 — スマホ幅で縦1カラム化（実機検証 iteration-2） */\n'
    '@media(max-width:768px){\n'
    '  /* 100vh フレックス/グリッドの高さチェーンを解放し通常スクロールに */\n'
    '  .db,#top,#body{display:block !important;height:auto !important;}\n'
    '  .db{min-height:100vh;}\n'
    '  div:has(> #body){display:block !important;height:auto !important;}\n'
    '  #body .panel{display:block !important;height:auto !important;max-height:none !important;}\n'
    '  #body .panel > div{height:auto !important;max-height:none !important;overflow:visible !important;}\n'
    '  /* 多カラムを縦並びに */\n'
    '  .mstrip{grid-template-columns:repeat(2,1fr) !important;}\n'
    '  .macro-cards-grid{grid-template-columns:repeat(2,1fr) !important;}\n'
    '  .val-macro-grid{grid-template-columns:repeat(2,1fr) !important;}\n'
    '  #modal-box{width:92vw !important;max-width:400px;}\n'
    '  /* ヘッダのロゴが1文字ずつ折り返す不具合を防ぐ */\n'
    '  #hdr{flex-wrap:wrap;}\n'
    '  #hdr-r{flex-wrap:wrap;}\n'
    '  #logo-text{white-space:nowrap;}\n'
    '  /* アコーディオン見出しは1行に収める */\n'
    '  #body .ph{white-space:nowrap;overflow:hidden;}\n'
    '  /* アコーディオン折りたたみ（@media 内限定＝PC では無効＝痕跡が残らない・手順書AB §2-B） */\n'
    '  #body .m-collapsed{display:none !important;}\n'
    '  /* 詳細パネルの3軸グリッドをスマホで1列に（手順書AB §2-C-3） */\n'
    '  .axis-grid{grid-template-columns:1fr !important;}\n'
    '  /* 銘柄テーブルを画面幅に収める（マトリクス .mx は除外＝§2-A で横スクロール対応・手順書AB §2-C-2） */\n'
    '  #body table:not(.mx){width:100% !important;}\n'
    '  #body table:not(.mx) th,#body table:not(.mx) td{white-space:normal !important;word-break:break-word;}\n'
    '}\n'
    '</style>'
)
if 'id="responsive-mobile"' in src:
    src = re.sub(r'<style id="responsive-mobile">.*?</style>',
                 lambda m: RESPONSIVE_CSS, src, count=1, flags=re.DOTALL)
    print("OK: レスポンシブ CSS 更新")
else:
    src = src.replace('</body>', RESPONSIVE_CSS + '\n</body>', 1)
    print("OK: レスポンシブ CSS 注入（モバイル対応）")

# ── モバイル アコーディオン（保有/監視/Top75 をタップ開閉・2026-05-22 CEO 指示）──
# スマホ幅では銘柄テーブルが縦に長すぎるため、見出し .ph をタップで開閉する。
# 既定は閉。PC（>768px）では何もしない。
MOBILE_ACCORDION_JS = '''<script id="mobile-accordion">
(function(){
  /* 手順書AB §2-B: 折りたたみは .m-collapsed クラス（@media 内限定）で行い、
     style.display は触らない＝PC 幅に戻れば自動で表示復活。
     matchMedia の change でブレークポイント往復のたび enable/teardown。 */
  var mq=window.matchMedia('(max-width:768px)');
  function targets(){
    return Array.prototype.filter.call(
      document.querySelectorAll('#body .ph'),
      function(ph){ return ph.id!=='d-ph' && ph.nextElementSibling
        && !ph.nextElementSibling.classList.contains('ph'); });
  }
  function enable(){
    targets().forEach(function(ph){
      if(ph.dataset.accBound==='1') return;
      ph.dataset.accBound='1';
      var c=ph.nextElementSibling;
      c.classList.add('m-collapsed');
      ph.style.cursor='pointer';
      if(!ph.querySelector('.m-acc-ind')){
        var ind=document.createElement('span');
        ind.className='m-acc-ind';
        ind.style.cssText='float:right;color:#f59e0b;font-weight:900;';
        ind.textContent='▼ 開く';
        ph.appendChild(ind);
      }
      ph._accH=function(){
        var open=!c.classList.toggle('m-collapsed');
        var ind=ph.querySelector('.m-acc-ind');
        if(ind) ind.textContent=open?'▲ 閉じる':'▼ 開く';
      };
      ph.addEventListener('click',ph._accH);
    });
  }
  function disable(){
    targets().forEach(function(ph){
      var c=ph.nextElementSibling;
      if(c) c.classList.remove('m-collapsed');
      if(ph._accH){ ph.removeEventListener('click',ph._accH); ph._accH=null; }
      ph.dataset.accBound='';
      ph.style.cursor='';
      var ind=ph.querySelector('.m-acc-ind');
...
if 'id="mobile-accordion"' in src:
    src = re.sub(r'<script id="mobile-accordion">.*?</script>',
                 lambda m: MOBILE_ACCORDION_JS, src, count=1, flags=re.DOTALL)
    print("OK: モバイル アコーディオン JS 更新")
else:
    src = src.replace('</body>', MOBILE_ACCORDION_JS + '\n</body>', 1)
    print("OK: モバイル アコーディオン JS 注入")

# ── 問題A: 銘柄選択時の横切れ修正（手順書AB §2-A・2026-05-22）─────────
# CSS Grid の 1fr トラックが grid item の min-width:auto でコンテンツ最小幅まで
# 膨張するのを minmax(0,1fr) で抑止。マトリクス表は横スクロール枠に隔離。
# @media の外＝全幅共通。main <style> より後に置きカスケードで上書き（!important 不要）。
LAYOUT_FIX_CSS = (
    '<style id="layout-fix">\n'
    '/* 問題A 修正 2026-05-22 手順書AB §2-A — grid トラック膨張の根治 */\n'
    '#body{grid-template-columns:minmax(0,1fr) minmax(0,1fr) minmax(0,1.05fr);}\n'
    '.mx-block{overflow-x:auto;}\n'
    'table.mx{min-width:420px;}\n'
    '</style>'
)
if 'id="layout-fix"' in src:
    src = re.sub(r'<style id="layout-fix">.*?</style>',
                 lambda m: LAYOUT_FIX_CSS, src, count=1, flags=re.DOTALL)
    print("OK: レイアウト修正 CSS 更新（問題A・横切れ根治）")
else:
    src = src.replace('</body>', LAYOUT_FIX_CSS + '\n</body>', 1)
    print("OK: レイアウト修正 CSS 注入（問題A・横切れ根治）")
