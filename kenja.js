function updateDetailLinks(c,n){
  ...
  s.innerHTML=
    '<div class="rb-t" style="color:#475569;">外部リンク</div>'
    ...
    +'<a href="'+ku+'" target="_blank" style="display:inline-flex;align-items:center;gap:5px;padding:5px 12px;background:#1e2d40;border:1px solid #374151;border-radius:5px;color:#94a3b8;font-size:10px;font-weight:700;text-decoration:none;">株探</a>'
    +'</div>'
    ...
    +'<span style="color:#f59e0b;font-weight:800;font-size:11px;">賢者の審判 - DEEP INSIGHT</span>'
    +'<button onclick="runKenja()" style="background:#f59e0b;color:#0d1117;border:none;border-radius:4px;padding:4px 12px;font-size:10px;font-weight:800;cursor:pointer;">▶ 分析開始</button>'
    +'</div>'
...
function kenjaPrompt(){
  return '賢者の審判：証券コード'
    +window._curCode+'（'+window._curName+'）の公開情報から分析してください。
...
function runKenja(){
  ...
  fetch('https://api.anthropic.com/v1/messages',{
    ...
    res.innerHTML='<div style="color:#e2e8f0;font-size:9px;line-height:1.7;white-space:pre-wrap;border-bottom:1px solid #374151;padding-bottom:10px;margin-bottom:8px;">'+escH(pA)+'</div>'
      +(pB?'<details open><summary style="color:#f59e0b;cursor:pointer;font-size:10px;font-weight:700;padding:4px 0;">詳細レポート (Part B)</summary><div style="color:#cbd5e1;font-size:9px;line-height:1.7;white-space:pre-wrap;margin-top:6px;">'+escH('[Part B]'+pB)+'</div></details>':'');
  }).catch(function(e){
    res.innerHTML='<div style="color:#f87171;padding:10px;">⚠ '+e.message+'<br>'
      +'<a href="https://disclosure2.edinet-fsa.go.jp/WZEK0040.aspx?S1='+code+'" target="_blank" style="color:#60a5fa;font-size:9px;">EDINETで直接確認</a></div>';
  });
}
