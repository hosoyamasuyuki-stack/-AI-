...
TOP_N = 150       # シートに保存する銘柄数（ダッシュボード表示は75社）
...
def calc_v43_score(df, price_info):
    ...
    # 2026-05-07 修正（CEO 指示・教訓 17 同種バグ）:
    # PEG は低いほど割安 → thr_low が正しい（thr_high は誤用）
    # weekly_update.py / manage_stock.py と整合させる
    # tests/test_scoring.py line 50-56 で誤動作を assert 固定済み（回帰テスト）
